 ...
 * 不做任何硬编码绝对路径（公开发布要求）。
 */
export function findInstall(dir, force = false) {
    if (dir)
        return markerExists(dir) ? dir : null;
    if (cachedInstall && !force)
        return cachedInstall;
    const candidates = [];
    if (process.env.NEUTRINO_DIR)
        candidates.push(process.env.NEUTRINO_DIR);
    if (isWindows()) {
...
export function markerExists(dir) {
    try {
        if (!existsSync(dir))
            return false;
...
export function installInfo(dir) {
...
export function listVoicebanks(dir = null) {
    ...
    for (const entry of readdirSync(modelDir, { withFileTypes: true })) {
        ...
        const bank = { name: entry.name, dir: bankDir };
        if (existsSync(infoFile)) {
            try {
                const info = parseToml(readFileSync(infoFile, 'utf8'));
                const speaker = info.speaker;
                bank.version = info.version;
                bank.type = info.type;
                bank.topKey = info.top_key;
                bank.bottomKey = info.bottom_key;
                bank.speaker = speaker?.name || entry.name;
                bank.gender = speaker?.gender;
                bank.language = speaker?.language;
...
export async function installVoicebank(zipPath, dir = null) {
    const install = dir || findInstall();
    if (!install)
...
async function moveBankDir(bankDir, modelDir, moved) {
    ...
    for (const e of entries) {
        if (e.isDirectory()) {
            if (await moveBankDir(path.join(bankDir, e.name), modelDir, moved))
                return true;
        }
    }
    return false;
...
// 目录与输出
// ---------------------------------------------------------------------------
export function scoreDir(dir = null) {
    const install = dir || findInstall();
    return install ? path.join(install, 'score', 'musicxml') : null;
}
export function outputDir(dir = null) {
    const install = dir || findInstall();
    return install ? path.join(install, 'output') : null;
}
export async function listOutputs(dir = null) {
...
export function latestOutput(dir = null) {
    const out = outputDir(dir);
    if (!out || !existsSync(out))
        return null;
