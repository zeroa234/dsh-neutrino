...
export function parseNote(token, divisionsPerBeat = 480) {
    token = token.trim();
    if (token === '')
        return null;
    if (token === ',' || token === '|') {
    ...
    if (!m)
        throw new Error(`Invalid note token: "${token}" (expected like C4:4:こ, C4:t1112:こ or R:4)`);
...
function durToDivisions(wholeUnits, dpb) {
    return Math.round(wholeUnits * 4 * dpb);
}
function divsToType(divs, dpb) {
    const beats = divs / dpb;
    const table = { '4': 'whole', '2': 'half', '1': 'quarter', '0.5': 'eighth', '0.25': '16th', '0.125': '32nd', '0.0625': '64th' };
    for (const [b, t] of Object.entries(table)) {
        if (Math.abs(Number(b) - beats) < 1e-6)
            return t;
    }
    for (const [b, t] of Object.entries(table)) {
        if (Math.abs(Number(b) * 1.5 - beats) < 1e-6)
            return t;
        if (Math.abs(Number(b) * 1.75 - beats) < 1e-6)
            return t;
    }
    let best = 'quarter';
...
function dotsFor(divs, dpb) {
    const beats = divs / dpb;
    for (const b of [4, 2, 1, 0.5, 0.25, 0.125]) {
        if (Math.abs(b - beats) < 1e-6)
            return 0;
        if (Math.abs(b * 1.5 - beats) < 1e-6)
            return 1;
        if (Math.abs(b * 1.75 - beats) < 1e-6)
            return 2;
    }
    return 0;
}
 ...
 * （后半段无歌词 → 自动 tie 延音）。
 */
export function buildMusicXML({ title = 'Untitled', notes = [], bpm = 120, beatsPerMeasure = 4, divisionsPerBeat = 480, voiceName = 'NEUTRINO' }) {
    ...
    for (let i = 0; i < parsed2.length; i++) {
        const n = parsed2[i];
        if (!n || n.type === 'rest' || n.type === 'breath')
            continue;
