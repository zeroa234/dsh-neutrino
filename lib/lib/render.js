 ...
 */
export function buildRenderParams(args, defaultModel) {
    const params = {};
    for (const k of Object.keys(args)) {
        ...
        if (v === undefined || v === null)
            continue;
        if (k === 'model')
            params.modelDir = String(v);
        else if (k === 'supportModel')
            params.supportModelDir = String(v);
        else if (RENDER_KEYS.has(k))
            params[k] = v;
    }
    if (defaultModel && !params.modelDir)
        params.modelDir = defaultModel;
    return params;
}
/**
 ...
 */
export async function render({ basename, params = {}, dir = null, onProgress }) {
    const install = dir || findInstall();
    if (!install)
        throw new NeutrinoError('NEUTRINO install not found');
    ...
    for (const f of [exe, toLabel]) {
        if (!existsSync(f))
            throw new NeutrinoError(`Missing binary: ${f}`);
    }
    // 决定声库（未指定时自动选第一个已安装）
    ...
    if (!modelDir) {
        ...
        if (!banks.length)
            throw new NeutrinoError('No voicebank installed in model/');
        modelDir = banks[0].name;
    }
    const modelPath = path.join(install, 'model', modelDir) + path.sep;
    if (!existsSync(modelPath))
        throw new NeutrinoError(`Voicebank not found: ${modelPath}`);
    ...
    if (!existsSync(scoreFile))
        throw new NeutrinoError(`Score not found: ${scoreFile}`);
    // step 1: score → label
    ...
    if (p.styleShift != null)
        args.push('-k', String(p.styleShift));
    if (p.transpose != null)
        args.push('-f', String(p.transpose));
    if (p.samplingRate != null)
        args.push('-s', String(p.samplingRate));
    if (p.bitDepth != null)
        args.push('-b', String(p.bitDepth));
    if (p.supportModelDir)
        args.push('-S', path.join(install, 'model', p.supportModelDir) + path.sep);
    if (p.skipTiming)
        args.push('--skip-timing');
    if (p.skipF0)
        args.push('--skip-f0');
    if (p.skipMelspec)
        args.push('--skip-melspec');
    if (p.skipWav)
        args.push('--skip-wav');
    if (p.phrase != null)
        args.push('-p', String(p.phrase));
    if (p.gpuId != null && p.gpuId >= 0)
        args.push('-g', String(p.gpuId));
    else if (p.useGPU !== false)
        args.push('-m'); // 最优 GPU（无 GPU 自动回落 CPU）
    if (p.phraseList)
        args.push('-i', p.phraseList);
...
function runExe(program, args, cwd, onProgress) {
    return new Promise((resolve) => {
        const child = spawn(program, args, { cwd, windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'] });
        let stdout = '';
        let stderr = '';
        child.stdout.on('data', (d) => { const s = d.toString('utf8'); stdout += s; onProgress?.(s); });
        child.stderr.on('data', (d) => { const s = d.toString('utf8'); stderr += s; onProgress?.(s); });
