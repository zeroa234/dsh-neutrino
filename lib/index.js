...
import { buildRenderParams, render } from './lib/render.js';
...
function formatRenderResult(result, extra = []) {
    const tail = 20;
    return [
        ...extra,
...
async function writeScoreFile(title, notes, opts) {
...
export function apply(ctx, config) {
    ...
    ctx.effect(() => ctx.tools.register(defineTool({
        ...
        async execute(args) {
            ...
            const tokens = Array.isArray(args.notes) ? args.notes : [];
            const fallback = expandHome(config.scoreDir);
            const { file } = await writeScoreFile(title, tokens, { bpm: args.bpm, beatsPerMeasure: args.beatsPerMeasure, install, fallbackDir: fallback });
            try {
